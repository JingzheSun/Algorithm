...
class TSP {
	// 2D array indexed by subset id subsetCount from (1-(n-1)) and destination
	// vertex j from (0-(n-1))
	Integer[][] A;
	...
	void run(int[][] graph) {
			...
			for (Integer sSet : sSets) {
				for (Integer vertex1 : sSetIdMap.get(sSet)) {
					...
					for (Integer vertex2 : sSetIdMap.get(sSet)) {
						...
						A[sSet][vertex1] = Math.min(A[sSet][vertex1],
								A[sSet ^ (1 << vertex1)][vertex2] + graph[vertex2][vertex1]);
					}
		...
		// out min value
		/**
		 * int min = Integer.MAX_VALUE; for (int i =1; i < maxSubsetSize; i++){
		 * if (min > A[subsetCount -1][i] + graph[i][0]){ min = A[subsetCount
		 * -1][i] + graph[i][0]; minFinalVertex = i; } } return min;
		 **/
